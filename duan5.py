# Modified version by Python GPT
import urllib3
import time
import random
import string
...
def sdtt(sdt):
    """
    Chuyển đổi số điện thoại Việt Nam: 0xxx -> +84xxx
    """
    if sdt.startswith("0"):
        return "+84" + sdt[1:]
    return sdt

def run(phone, count, input_key="trong288"):
    """
    Hàm chính được gọi bởi runner.py để thực thi script duan2
    """
    try:
        count = int(count)
        if count > 10000:
            count = 10000  # Giới hạn số lần tránh treo server
    except ValueError:
        return "[duan5] Số lần không hợp lệ."

    phone = sdtt(phone)
    results = [
        f"[duan2] Số điện thoại: {phone}",
        f"[duan2] Số lần lặp: {count}"
    ]

    for i in range(1, count + 1):
        msg = f"[duan2] Lần {i}: Gửi đến {phone}"
        print(msg)
        results.append(msg)
        time.sleep(0.05)  # nhẹ nhàng mô phỏng tiến trình
