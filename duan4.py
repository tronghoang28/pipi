...
# Các hàm spam giả lập — thay bằng API thật nếu cần
def tv360(phone):
    print(f"[tv360] Gửi spam đến {phone}")
    time.sleep(0.1)

def robot(phone):
    print(f"[robot] Gửi spam đến {phone}")
    time.sleep(0.1)

# Danh sách các hàm spam
spam_functions = [tv360, robot]

# Hàm chính gọi từ runner
def run(phone, i, input_key="trong288"):
    if not verify_key(input_key):
        return f"[duan4] Key không hợp lệ ở lần {i}"

    phone = format_phone(phone)
    log_header = f"[duan4] Lần {i} - {strftime('%H:%M:%S')}:"

    results = [f"{log_header} Đang spam đến {phone}"]
    print(results[0])

    for func in spam_functions:
        try:
            func(phone)
            results.append(f"{log_header} {func.__name__} OK")
        except Exception as e:
            error_msg = f"{log_header} {func.__name__} lỗi: {e}"
            print(error_msg)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=30) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                print(f'Exception xảy ra: {exc}')

    time.sleep(1)
    return f"Hoàn thành lần {i}"
