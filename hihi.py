...
# ============ CONFIG ============
BOT_TOKEN = "7693427260:AAEerxeouE1hgMaWxqL4K14nUQbLgPYBjfI"
...
async def auto_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    get_permission_title(user_id)

    # Kiểm tra VIP
    if not is_vip(int(user_id)):
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💬 Đầu Khấc! 💸", switch_inline_query_current_chat="/trialvip")]
    ])
    return await update.message.reply_text(
        "<blockquote>🐸➤Hiện tại bạn chưa có quyền VIP\n"
        "Ấn vào nút bên dưới để yêu cầu dùng thửi</blockquote>",
        parse_mode="HTML", 
        reply_markup=kb
    )
